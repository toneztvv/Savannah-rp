...
`RUN-DISCORD-SETUP.bat` (+ `discord-setup.ps1`) builds **all the roles, all the
categories, all the channels, and every permission** for you via the Discord API.

1. Make a bot: <https://discord.com/developers/applications> → New Application →
   **Bot** → **Reset Token** → copy it.
2. Invite it with **Administrator**: OAuth2 → URL Generator → scope `bot`,
   permission `Administrator` → open the link → add to your server. Then in
   **Server Settings → Roles**, drag the bot's role near the **top**.
3. Get your Server ID: Discord Settings → Advanced → **Developer Mode ON**,
   right-click the server icon → **Copy Server ID**.
4. Double-click **`RUN-DISCORD-SETUP.bat`**, paste the token and server ID.

It's **safe** — it only adds, skips anything that already exists, and you can
re-run it. Then do the **bots** section below by hand (verification, tickets,
Tebex) — those can't be scripted.

The rest of this file is the manual reference / who-can-see-what breakdown.
